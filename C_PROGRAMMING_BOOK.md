...
Strings are sequences of characters that represent text. In C, strings have a unique implementation compared to other languages - they're simply arrays of characters terminated by a special character called the **null terminator** (`'\0'`). Understanding strings is crucial for any practical C programming, and essential for WinRDP which handles hostnames, descriptions, usernames, and passwords.
...
- Wide characters for Unicode (Windows requirement)
- Common string pitfalls and how to avoid them

### Why This Matters for WinRDP
WinRDP deals with strings constantly:
- Hostnames: `"server1.company.com"`
- Descriptions: `"Production Database Server"`
- Usernames: `"admin"`
- File paths: `"C:\\Users\\Documents\\hosts.csv"`
- Wide strings for Windows API: `L"Server Name"`
...
int main(void)
{
    ...
    char greeting[] = "Welcome!";  // String literal initializes array
...
- They automatically include a null terminator `'\0'`
- You can't modify string literals (undefined behavior)

### Character Arrays as Strings

Strings in C are character arrays with a `'\0'` at the end:
...
{
    char str1[10] = "Hi";
    char str2[10] = {'H', 'i'};  // Missing '\0'!
    
    printf("str1: %s\n", str1);  // Safe: "Hi"
    printf("str2: %s\n", str2);  // UNDEFINED BEHAVIOR! No null terminator
    
    // Correct way with explicit null terminator:
    char str3[10] = {'H', 'i', '\0'};
    printf("str3: %s\n", str3);  // Safe: "Hi"
...
**Critical Rule:** Always ensure your strings have a null terminator!
...
int main(void)
{
    ...
    printf("Enter your name: ");
    scanf("%s", name);  // No & needed! (name is already a pointer)
...
**Limitation:** `scanf("%s", ...)` stops at whitespace!

**Using `fgets` (entire line):**
...
- Prevents buffer overflow (size limit)
...
**Danger:** `strcpy` doesn't check buffer size!

**`strncpy` - Safer (copies up to n characters):**
...
int main(void)
{
    ...
    if (found != NULL)
    {
        printf("Found 'quick' at position: %ld\n", found - text);
        printf("From 'quick' onward: %s\n", found);
    }
...
From 'quick' onward: quick brown fox jumps
```

### String Tokenization: `strtok`

Splits a string into tokens (parts) based on delimiters:
...
int my_strcmp(const char *str1, const char *str2)
{
    ...
    // If we reach here, check if one string is longer
    return str1[i] - str2[i];
}
...
## Wide Strings (Windows Unicode)

Windows uses **wide characters** (`wchar_t`) for Unicode support. This is crucial for international text.
...
int main(void)
{
    ...
    // Wide string (Unicode) - note the L prefix
    wchar_t wide[] = L"Hello";
    
    // Wide string with special characters
    wchar_t unicode[] = L"Hello 世界 🌍";
...
int main(void)
{
    ...
    if (wcscmp(str1, L"Hello, World!") == 0)
    {
        wprintf(L"Strings match!\n");
    }
...
### Why WinRDP Uses Wide Strings

Windows API functions come in two versions:
- **ANSI version** (`CreateFileA`, `MessageBoxA`) - Uses `char*`
- **Unicode version** (`CreateFileW`, `MessageBoxW`) - Uses `wchar_t*`

**Modern Windows applications should use Unicode (wide strings)!**

```c
// WinRDP style - always use wide strings
wchar_t hostname[256];
wcscpy(hostname, L"server1.company.com");

// Windows API call
MessageBoxW(NULL, hostname, L"Server", MB_OK);
```
...
int main(void)
{
    ...
    printf("Whitespace only: %d\n", isEmptyOrWhitespace("   "));
    printf("Has content: %d\n", isEmptyOrWhitespace("Hello"));
...
int main(void)
{
    ...
    printf("Words in 'The quick brown fox': %d\n", countWords("The quick brown fox"));
    printf("Words in '  Multiple   spaces  ': %d\n", countWords("  Multiple   spaces  "));
...
### Exercise 7.5: String Split (Simple CSV)

Write a function that splits a CSV line into fields:
...
int main(void)
{
    char line[] = "server1,Production Server,192.168.1.100";
    char *fields[10];
    
    int count = splitCSV(line, fields, 10);
...
char small[5];
strcpy(small, "This is too long");  // DANGER!

// CORRECT - Check size
char small[5];
strncpy(small, "This is too long", sizeof(small) - 1);
small[sizeof(small) - 1] = '\0';  // Ensure null terminator
```
...
```c
// WRONG - String literals are read-only!
char *str = "Hello";
...
### Mistake 5: Using `strlen` on Non-Strings

```c
// WRONG - No null terminator
char notAString[5] = {'H', 'e', 'l', 'l', 'o'};
int len = strlen(notAString);  // UNDEFINED! Reads past array

// CORRECT - Null-terminate
char isAString[6] = {'H', 'e', 'l', 'l', 'o', '\0'};
int len = strlen(isAString);  // Safe: 5
```

## Connection to WinRDP

### How WinRDP Uses Strings
...
**2. CSV Parsing:**
```c
// Reading line from hosts.csv
wchar_t line[1024];
fgetws(line, 1024, file);
...
{
    MessageBoxW(hDlg, L"Hostname cannot be empty!", L"Error", MB_OK);
    return;
}
```

**4. Building File Paths:**
```c
wchar_t path[MAX_PATH];
GetDocumentsFolder(path, MAX_PATH);
PathCombineW(path, MAX_PATH, path, L"hosts.csv");
```

## Summary

You've learned:
- ✅ Strings are character arrays with null terminators
- ✅ String literals vs character arrays
- ✅ Essential string functions (`strlen`, `strcpy`, `strcmp`, etc.)
- ✅ Building custom string functions
- ✅ Wide strings for Unicode (Windows)
- ✅ Practical operations (trim, uppercase, search)
- ✅ Common string pitfalls and how to avoid them

**Key Takeaways:**
1. **Always null-terminate** your strings
2. **Check buffer sizes** to prevent overflow
3. **Use `strcmp`** to compare strings, never `==`
4. **Wide strings** (`wchar_t`) for Windows API
5. **Validate user input** (trim whitespace, check empty)

**Next chapter:** We'll learn about structures to organize related data (like hostname + description together)!
...
So far, we've worked with individual pieces of data: integers, floats, characters, arrays. But real programs deal with complex entities that have multiple attributes. A person has a name, age, and address. A book has a title, author, and ISBN. An RDP server has a hostname and description.

**Structures** allow us to group related data together under a single name. They're essential for organizing complex data and are used extensively in C programming and Windows API.
...
- Nested structures
- `typedef` for cleaner code
- How WinRDP uses structures

### Why This Matters for WinRDP
WinRDP defines several structures:
- `Host` - Stores hostname and description
- `NOTIFYICONDATA` - Windows system tray icon
- `LVITEM` - Windows ListView item
- Credential data organization

## Defining a Structure

A **structure** is a user-defined data type that groups variables:
...
int main(void)
{
    struct Person person1;  // Create variable
    struct Person person2;  // Create another
...
struct Person
{
    char name[50];
    int age;
} person1, person2;  // Create variables here

int main(void)
{
    // person1 and person2 already exist
...
int main(void)
{
    ...
    printPerson(person);  // Copies entire structure
...
{
    // Use -> operator for pointer to structure
    printf("Name: %s, Age: %d\n", p->name, p->age);
}
...
int main(void)
{
    ...
    printPerson(&person);  // Pass address (efficient!)
...
**Key difference:**
- `.` for structure variables: `person.name`
- `->` for structure pointers: `ptr->name`
...
int main(void)
{
    struct Person person = createPerson("Bob", 25);
...
// Won't work - modifies copy
void incrementWrong(struct Counter c)
{
    c.value++;  // Only changes local copy!
}

// Works - modifies original
void incrementRight(struct Counter *c)
{
    c->value++;  // Changes original via pointer
}
...
int main(void)
{
    ...
    incrementWrong(counter);
    printf("After incrementWrong: %d\n", counter.value);  // Still 0!
    
    incrementRight(&counter);
    printf("After incrementRight: %d\n", counter.value);  // Now 1
...
int main(void)
{
    ...
    // Method 1: Dereference then use dot
    printf("Name: %s\n", (*ptr).name);
...
int main(void)
{
    struct Person person1;  // Must write "struct Person"
...
// Define and create typedef simultaneously
typedef struct
{
    char name[50];
    int age;
} Person;  // Now "Person" is the type name

int main(void)
{
    Person person1;  // Much cleaner!
    Person person2;
...
**This is the preferred style in modern C!**

## WinRDP's Host Structure

Here's how WinRDP defines the Host structure:
...
// Create a host
Host newHost;
wcscpy(newHost.hostname, L"server1.company.com");
wcscpy(newHost.description, L"Production Server");
...
// Add host to array
wcscpy(hosts[hostCount].hostname, L"server1");
wcscpy(hosts[hostCount].description, L"Web Server");
hostCount++;

// Access host
printf("Host: %ls - %ls\n", hosts[0].hostname, hosts[0].description);
```
...
int main(void)
{
    ...
    float avg = calculateAverage(class, 3);
...
### Example 2: Rectangle Operations
...
int getRectangleArea(const Rectangle *rect)
{
    return getRectangleWidth(rect) * getRectangleHeight(rect);
}
...
int main(void)
{
    ...
    printf("Width: %d\n", getRectangleWidth(&rect));
    printf("Height: %d\n", getRectangleHeight(&rect));
...
int main(void)
{
    ...
    // Add contacts
    addContact(contacts, &count, "Alice", "alice@example.com", "555-0101");
    addContact(contacts, &count, "Bob", "bob@example.com", "555-0102");
    addContact(contacts, &count, "Charlie", "charlie@example.com", "555-0103");
    ...
    Contact *found = findContact(contacts, count, "Bob");
    if (found != NULL)
    {
        printf("Found Bob:\n");
        printContact(found);
...
### Exercise 8.1: Book Structure

Create a book structure and functions to manage books:
...
typedef struct
{
    // Your code: Add title, author, year, price
} Book;

void printBook(const Book *b)
{
    // Your code: Print book details
}

int main(void)
{
    Book book = {"The C Programming Language", "K&R", 1978, 59.99};
...
### Exercise 8.2: Find Most Expensive Book

Given an array of books, find the most expensive:
...
Book* findMostExpensive(Book books[], int count)
{
    // Your code here
}
...
int main(void)
{
    ...
    Book *expensive = findMostExpensive(books, 3);
...
### Exercise 8.3: Sort Students by GPA

Sort an array of students by GPA (descending):
...
{
    // Your code: Use bubble sort or any sorting algorithm
}
...
### Exercise 8.4: Employee Database
...
{
    // Your code: Increase salary by percentage
}

float calculateTotalPayroll(const Employee employees[], int count)
{
    // Your code: Sum all salaries
}

void printHighEarners(const Employee employees[], int count, float threshold)
{
    // Your code: Print employees with salary >= threshold
}
...
int main(void)
{
    ...
    printf("Total payroll: $%.2f\n", calculateTotalPayroll(employees, count));
    
    giveRaise(&employees[1], 10.0);  // 10% raise for Bob
    printf("Bob's new salary: $%.2f\n", employees[1].salary);
    
    printf("\nHigh earners (>= $70000):\n");
    printHighEarners(employees, count, 70000);
...
### Mistake 1: Forgetting `struct` Keyword (Without typedef)
...
// WRONG
Person p;  // Error! "Person" is not defined

// CORRECT
struct Person p;  // Must use "struct" keyword

// OR use typedef from the start
typedef struct
{
    char name[50];
    int age;
...
Person p;  // Now this works!
```

### Mistake 2: Using `.` Instead of `->` (and vice versa)
...
printf("%d\n", person->age);  // Error! person is not a pointer
printf("%d\n", ptr.age);      // Error! ptr is a pointer

// CORRECT
printf("%d\n", person.age);   // Use . for structure variable
printf("%d\n", ptr->age);     // Use -> for pointer to structure
```

### Mistake 3: Comparing Structures with `==`
...
// WRONG - Can't compare structures with ==
...
bool pointsEqual(struct Point a, struct Point b)
{
    return a.x == b.x && a.y == b.y;
}
```

### Mistake 4: Not Initializing Structures
...
// WRONG - Uninitialized structure (garbage values)
struct Person person;
printf("%s, %d\n", person.name, person.age);  // UNDEFINED!

// CORRECT - Initialize
struct Person person = {"Alice", 30};
// OR
struct Person person = {0};  // Zero-initialize all members
```

### Mistake 5: Returning Pointer to Local Structure
...
struct Person createPerson(const char *name, int age)
{
    ...
    strcpy(p.name, name);
    p.age = age;
    return p;  // Safe - copies structure
}

// OR use dynamic memory (covered in next chapter)
struct Person* createPerson(const char *name, int age)
{
    struct Person *p = malloc(sizeof(struct Person));
    strcpy(p->name, name);
    p->age = age;
    return p;  // Safe - dynamically allocated
}
```

## Connection to WinRDP

### Windows Structures in WinRDP
...
lvi.iItem = 0;
lvi.iSubItem = 0;
lvi.pszText = L"server1.company.com";

SendMessageW(hListView, LVM_INSERTITEMW, 0, (LPARAM)&lvi);
```

**3. WinRDP's Host Array:**
```c
typedef struct
{
    wchar_t hostname[256];
...
// Dynamic array of structures (next chapter!)
```

## Structure Memory Layout

Understanding how structures are stored in memory:
...
int main(void)
{
    ...
    // Might print 12, not 7!
    // Due to memory alignment (padding)
    
    return 0;
}
```

**Memory alignment** adds padding for efficiency. The compiler arranges members optimally.
...
- ✅ Passing structures to functions (by value and pointer)
- ✅ Arrays of structures
- ✅ Nested structures
- ✅ Using `typedef` for cleaner code
- ✅ How Windows API uses structures extensively

**Key Takeaways:**
1. **Structures organize** related data under one name
2. **Use `.`** for structure variables: `person.age`
3. **Use `->`** for structure pointers: `ptr->age`
4. **Pass by pointer** for efficiency (avoid copying large structures)
5. **`typedef`** makes code cleaner and more readable
6. **Windows structures** (like `NOTIFYICONDATAW`) follow the same principles

**You now have:**
- Complete C fundamentals (Part I finished!)
- Understanding of all basic C concepts
- Foundation for Windows programming
- Ready to learn advanced topics!

**Next chapters:** We'll learn dynamic memory allocation (Chapter 9), file I/O (Chapter 10), and then build WinRDP's host management system (Chapter 19)!
